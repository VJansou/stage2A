...
def process_4_images(dataset, diff_values):
    ...
    # Utiliser la svd
    for i in range(3):
        U, S, Vt = np.linalg.svd(image_pyramid[3][i], full_matrices=False)
        S_trunc = np.zeros_like(S)
        S_trunc[:20] = S[:20]
        image_pyramid[3][i] = U @ np.diag(S_trunc) @ Vt

    # Faire une moyenne des 3 premières images
    I3 = (image_pyramid[3][0] + image_pyramid[3][1] + image_pyramid[3][2])/3
    I4 = image_pyramid[3][3]
