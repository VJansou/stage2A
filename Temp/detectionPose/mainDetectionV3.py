...
def process_4_images(dataset, diff_values):
    ...
    I3 = utils.create_I_matrix(image_pyramid[3][:3])
    I4 = utils.create_I_matrix(image_pyramid[3])

    # Calculer la SVD de la matrice I
    S3 = np.linalg.svd(I3, full_matrices=False, compute_uv=False)
    S4 = np.linalg.svd(I4, full_matrices=False, compute_uv=False)

    # Déterminer si l'écart dépasse le seuil
    diff = np.linalg.norm(sum(S3) - sum(S4)) / np.linalg.norm(sum(S3))
...
def main():
    ...
    while continue_processing:
        ...
        if num_image + 4 <= nb_images:
            start_indices.append(num_image + 4)
